        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
              ...
              var result = {
                name: name,
                location: {
                  latitude: (response.geojson.bbox[2] - response.geojson.bbox[0]) / 2 + response.geojson.bbox[2],
                  longitude: (response.geojson.bbox[3] - response.geojson.bbox[1]) / 2 + response.geojson.bbox[3]
              }};
              console.log(result);
              results.push(result);
            }
