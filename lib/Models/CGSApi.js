        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
              ...
              results.push({
                name: name,
                location: {
                  latitude: (response.geojson.bbox[0][2] - response.geojson.bbox[0][0]) / 2 + response.geojson.bbox[0][2],
                  longitude: (response.geojson.bbox[0][3] - response.geojson.bbox[0][1]) / 2 + response.geojson.bbox[0][3]
                }
              });
            }
        };
